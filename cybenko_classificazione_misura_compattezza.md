...
Questo documento raccoglie e integra le spiegazioni sviluppate nella conversazione sul teorema di approssimazione universale e sulla sua estensione ai problemi di classificazione. L'obiettivo è introdurre ogni simbolo prima di usarlo e costruire gradualmente i concetti matematici necessari: compattezza, ricoprimenti, successioni, norma uniforme, sigma-algebre, insiemi misurabili, misura e integrale di Lebesgue, convergenza uniforme e teorema di Lusin.
...
Nello stesso articolo, Cybenko presenta anche un risultato per le **funzioni di decisione**, cioè funzioni che associano a ogni input una classe. Poiché una funzione di classificazione è normalmente discontinua sulle frontiere tra le classi, il risultato non può essere formulato come approssimazione uniforme arbitrariamente precisa su ogni punto del dominio. La garanzia è invece valida su un sottoinsieme che occupa quasi tutto il dominio, salvo un insieme di misura arbitrariamente piccola.
...
L'interpretazione geometrica è semplice: la distanza tra \(G(x)\) e l'etichetta concorrente \(\ell\) non può essere inferiore alla distanza tra \(j\) e \(\ell\), meno lo spostamento di \(G(x)\) rispetto a \(j\).
...
Qualunque sua sottosuccessione converge ancora a \(0\), perché gli indici selezionati continuano a crescere verso infinito. Quindi questa successione non possiede una sottosuccessione convergente a un punto di \((0,1)\).
...
> scelto un volume massimo \(\eta>0\), piccolo quanto desideriamo, possiamo trovare una rete per la quale il controllo uniforme dell'errore vale fuori da un insieme \(E\) avente volume inferiore a \(\eta\).
